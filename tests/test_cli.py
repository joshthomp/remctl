...
class AccountScopeIsolationMixin:
    """Isolate tests from the developer's real remctl config, the
    REMCTL_* environment variables, and module-level account caches so
    account-scope behavior is deterministic regardless of local state.

    Without this, a real ~/.config/remctl/config.json containing
    {"accountScope": "all"} silently pushes commands down the
    multi-account path during tests.
    """

    def setUp(self):
        remctl = self.remctl
        self._saved_store_dir = remctl.STORE_DIR
        self._saved_db_override = remctl.DB_OVERRIDE
        self._saved_config_file = remctl.CONFIG_FILE
        ...
        remctl.CONFIG_FILE = Path("/nonexistent/remctl-test-config.json")
        remctl._ACCOUNT_CACHE = None
        try:
    ...
    def tearDown(self):
        super().tearDown()
        remctl = self.remctl
        remctl.STORE_DIR = self._saved_store_dir
        remctl.DB_OVERRIDE = self._saved_db_override
        remctl.CONFIG_FILE = self._saved_config_file
        remctl._ACCOUNT_CACHE = None
        for var, val in (
...
class CliTests(AccountScopeIsolationMixin, unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_discover_accounts_excludes_hidden_and_empty_stores(self):
        ...
        with (
            ...
            # discover_accounts ranks with the same scorer as find_main_db_path();
            # a constant score keeps this test about filtering, not ordering.
            mock.patch.object(
                self.remctl, "reminders_db_score",
                side_effect=lambda path: (1, fake_reminder_count(path), 0, 0, 0, 0.0, 0, 0),
            ),
        ):
    ...
    def test_cmd_add_with_account_opens_correct_store_for_list_lookup(self):
        ...
        with (
            ...
            mock.patch.object(
                self.remctl, "bridge_call_result",
                return_value=self._bridge_result({"status": "created", "id": "NEW-001"}),
            ),
            mock.patch.object(self.remctl, "q_reminder_by_identifier", return_value=None),
    ...
    def test_cmd_add_without_account_uses_open_db(self):
        ...
        with (
            ...
            mock.patch.object(
                self.remctl, "bridge_call_result",
                return_value=self._bridge_result({"status": "created", "id": "NEW-002"}),
            ),
            mock.patch.object(self.remctl, "q_reminder_by_identifier", return_value=None),
            mock.patch.object(self.remctl, "private_metadata_enabled", return_value=False),
            contextlib.redirect_stdout(io.StringIO()),
    ...
    def test_ek_identifier_falls_back_to_find_reminder_for_exchange(self):
        ...
        with (
            ...
            mock.patch.object(self.remctl, "bridge_call_result",
                              side_effect=lambda data: self._bridge_result(fake_bridge_call(data))),
        ):
    ...
    def test_cmd_add_account_with_list_passes_calendar_identifier(self):
        ...
        # The list title/listId are still sent as a fallback for older bridges;
        # findList() in the bridge prefers calendarIdentifier when present.
        self.assertEqual(create_payloads[0].get("list"), "Tasks")
    ...
    def test_cmd_add_ambiguous_list_without_account_exits(self):
        """add -l Tasks under an all-accounts scope errors when Tasks exists in two accounts.

        Ambiguity detection only applies once the scope spans more than one
        account; the default single-account path never pays for the extra
        list_calendars probe.
        """
        def fake_bridge_call(data):
            if data["action"] == "list_calendars":
        ...
        with (
            ...
            args = self._add_args(title="T", list="Tasks", account=None)
...
class InlineImageTests(AccountScopeIsolationMixin, unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.remctl = load_module("remctl_cli_test", "remctl")
...
class ImageFlagParsingTests(AccountScopeIsolationMixin, unittest.TestCase):
...
class TrailingBadgeTests(AccountScopeIsolationMixin, unittest.TestCase):
